...
export class HomeView {
  ...
  private buildHomeSection(): HTMLElement {
    ...
    homeSection.innerHTML = this.homeHtmlTemplate();
    return homeSection;
  }

  /**
   * Returns the HTML template for the home section.
   * @returns The HTML string for the home section.
   */
  private homeHtmlTemplate(): string {
    return /* html */ `
  ...
  private insertButton(homeSection: HTMLElement): void {
    const btnContainer: HTMLElement | null = homeSection.querySelector('.home-section__home-btn-container');
    if (btnContainer) {
